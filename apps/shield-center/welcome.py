...
class Welcome(tk.Tk):
    def __init__(self) -> None:
        ...
        ttk.Label(root, text="Developer Preview 0.1", style="Accent.TLabel").pack(anchor="w", pady=(4, 22))
        ttk.Label(
            root,
            text=(
                "ShieldOS is a clean, private desktop system. It uses local accounts by default, "
                "keeps the firewall enabled, includes app sandboxing tools, and gives you one "
                "simple Shield Center for privacy, gaming and developer controls."
            ),
            wraplength=660,
        ).pack(anchor="w", pady=(0, 20))
        ttk.Label(root, text="Live user: shield  |  Live password: shield").pack(anchor="w", pady=3)
        ttk.Label(root, text="Open Shield Center from the desktop or app launcher.").pack(anchor="w", pady=3)
        ttk.Label(root, text="Build command: nix build .#nixosConfigurations.shieldos-iso.config.system.build.isoImage").pack(anchor="w", pady=3)
